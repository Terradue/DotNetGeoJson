...
namespace Terradue.GeoJson.Tests
{
  ...
  public class GeoRSSTest
  {
    ...
    public void GeoRssFromAtomFeed()
    {
      ...
      atomf.ReadFrom(XmlReader.Create(new FileStream("Samples/landsat8.xml".TestDirectory(), FileMode.Open, FileAccess.Read)));
    ...
    {
      var xr = XmlReader.Create(new FileStream("Samples/georsswhere.xml".TestDirectory(), FileMode.Open, FileAccess.Read));
    ...
    {
      var xr = XmlReader.Create(new FileStream("Samples/noa-ers-georss.xml".TestDirectory(), FileMode.Open, FileAccess.Read));

      var geom = GeoRssHelper.Deserialize(xr).ToGeometry() as MultiPolygon;

      Assert.IsNotNull(geom);

      xr = XmlReader.Create(new FileStream("Samples/noa-ers-georss.xml".TestDirectory(), FileMode.Open, FileAccess.Read));
    ...
    {
      var xml =
        "<georss:polygon xmlns:georss=\"http://www.georss.org/georss\">45.256 -110.45 46.46 -109.48 43.84 -109.86 45.256 -110.45</georss:polygon>";
    ...
    {
      var xml =
        "<?xml version=\"1.0\" encoding=\"utf-16\"?><georss:point xmlns:georss=\"http://www.georss.org/georss\">45.256 -71.92</georss:point>";
    ...
    {
      var fs = new FileStream("Samples/multisurface32-2.xml".TestDirectory(), FileMode.Open, FileAccess.Read);
      ...
      doc.Save("out/georssfromgml32.xml".TestDirectory());
    }
  }
}