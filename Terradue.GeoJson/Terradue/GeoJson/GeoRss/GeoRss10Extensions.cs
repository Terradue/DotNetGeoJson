...
namespace Terradue.GeoJson.GeoRss10
{
  public static class GeoRss10Extensions
  {
    ...
    {
      if (georss is GeoRss10Point) return ((GeoRss10Point) georss).ToGeometry();

      if (georss is GeoRss10Line) return ToGeometry((GeoRss10Line) georss);

      if (georss is GeoRss10Polygon) return ToGeometry((GeoRss10Polygon) georss);

      if (georss is GeoRss10Box) return ToGeometry((GeoRss10Box) georss);

      if (georss is GeoRss10Where) return ToGeometry((GeoRss10Where) georss);
    ...
    {
      if (where.Item is EnvelopeType) throw new NotImplementedException();

      if (where.Item is CircleByCenterPointType) throw new NotImplementedException();

      if (where.Item is LineStringType) return ((LineStringType) where.Item).ToGeometry();

      if (where.Item is PointType) return ((PointType) where.Item).ToGeometry();

      if (where.Item is PolygonType) return ((PolygonType) where.Item).ToGeometry();
