...
//        }
    ...
    public static GeometryObject SplitWorldExtent(Polygon poly)
    {
      ...
      foreach (var lineString in poly.LineStrings.Skip(1)) InnerlineStrings.AddRange(SplitWorldExtent(lineString));

      if (OuterlineStrings.Count == 1) return poly;

      var newpolygons = new List<Polygon>();

      if (OuterlineStrings.Count <= 0) return newpoly;
